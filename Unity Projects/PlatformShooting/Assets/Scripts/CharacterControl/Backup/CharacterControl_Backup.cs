...
public class CharacterControl_Backup : MonoBehaviour
{
    ...
    private WeaponControl_Backup _weaponControl;
    ...
    void Awake()
    {
        ...
        _weaponControl = GetComponent<WeaponControl_Backup>();
    ...
    private void SwitchTarget(Rigidbody suspect)
    {
        ...
        if (_isNeutral) // I'm a neutral character
        {
            ...
            } else if (!suspect.CompareTag(_neutralTag)) // Currently has team target & suspect is not neutral
            {
                ...
                {
                    _targetCharacter = suspect.gameObject;
                } else if (!TargetInRange(_targetCharacter.transform.position, WeaponControl_Backup.ShootRange)) // Current target is out of shoot range
                {
    ...
    private void ChaseTarget()
    {
        ...
        if (!TargetInRange(targetPosition, 0.5f * WeaponControl_Backup.ShootRange) && !ObstacleBetween(targetPosition))
        {
   ...
   private void ZeroHealth()
    {
        ...
        if (_targetCharacter != null)
        {
            _isNeutral = false;
            FullHealth();
            _targetCharacter.GetComponent<CharacterControl_Backup>().FullHealth();
